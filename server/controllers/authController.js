...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    console.log("reset token: " + resetToken);// to test the email sending until fixing the mail service
    await user.save({ validateBeforeSave: false }); // to not validate the password

    // 3) Send it to user's email
    const resetUrl = `${req.protocol}://${req.get("host")}/api/resetPassword/${resetToken}`;
    const message = `
    For a password reset request, submet a patch request to the following url: ${resetUrl} with your new password and password confirmation
